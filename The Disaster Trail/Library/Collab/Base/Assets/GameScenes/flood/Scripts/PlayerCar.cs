...
public class PlayerCar : MonoBehaviour
{
	...
	void TakeDamage()
	{
		...
		buffer = Time.time;
		if ((Mathf.Ceil(health / 10) * 10 - 20) % 30 == 0)
		{
			speedMultiplier *= .8f;
		...
		//TODO: ^these debuffs need to be added when they pass thresholds like 70, 40, etc.
		//play crash sound - DO THIS ^ HERE
	}

	void GameOver()
	{
		// GameManager.instance.FadeToBlack(GameManager.instance.FadeTransitionSpeedPerFrame);
		GameManager.instance.FadeToBlackAndLoad(GameManager.instance.FadeTransitionSpeedPerFrame, 0);
		// GameManager.instance.LoadScene(0,2);
		maxSpeed = 0;
